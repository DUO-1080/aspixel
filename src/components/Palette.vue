<template>
  <div class="flex-grow w-full bg-indigo-900">
    <div class="flex flex-wrap">
      <div class="palette-item w-8 h-8 relative cursor-pointer border border-black" v-for="color in palette"
           :style="{backgroundColor: `#${color}`}" :key="color" @click="setCurrentColor(color)">
        <div v-if="currentColor.replace('#', '').toLowerCase() === color"
...
export default {
  ...
  setup(props) {
    ...
    const palette = ref(props.palette.colors);
    const currentColor
